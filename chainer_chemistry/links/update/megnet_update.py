...
def get_concated_edge_vec(atom_feat, pair_feat, global_feat,
                          bond_num, bond_idx, xp):
    ...
    base_idx = xp.broadcast_to(
        xp.arange(0, p0 * a1, a1).reshape(-1, 1), (p0, p1))
    ...
    concated_vec = functions.concat((reshaped_pair_feat,
                                     start_node, end_node,
                                     reshaped_global_feat))

    # masking
    mask = xp.zeros((p0 * p1), dtype=xp.bool)
    for i in range(p0):
        mask[xp.arange(p1 * i, (p1 * i + bond_num[i]))] = True
...
def get_concated_node_vec(atom_feat, pair_feat, global_feat,
                          atom_num, bond_num, bond_idx, xp):
    a0, a1, a2 = atom_feat.shape
    p0, p1, p2 = pair_feat.shape
    g0, g1 = global_feat.shape
    ...
    mean_edge_vec = sum_edeg_vec / functions.broadcast_to(
        degree.reshape(-1, 1), sum_edeg_vec.shape)
    ...
    for i in range(a0):
        mask[xp.arange(a1 * i, (a1 * i + atom_num[i]))] = True
    mask = xp.broadcast_to(mask.reshape(-1, 1), concated_vec.shape)
    mask_val = xp.zeros(concated_vec.shape, dtype=xp.float32)
    masked_vec = functions.where(mask, concated_vec, mask_val)
...
class MEGNetUpdate(chainer.Chain):
    ...
    def __init__(self, hidden_dim_for_dense=[64, 32],
        ...
        with self.init_scope():
            # for dense layer
            self.dense_for_atom = DenseLayerWithGraphLinear(
                hidden_dim=hidden_dim_for_dense)
            self.dense_for_pair = DenseLayerWithGraphLinear(
                hidden_dim=hidden_dim_for_dense)
            self.dense_for_global = DenseLayerWithLinear(
                hidden_dim=hidden_dim_for_dense)
            # for update layer
            self.update_for_atom = UpdateLayerWithGraphLinear(
                hidden_dim=hidden_dim_for_update)
            self.update_for_pair = UpdateLayerWithGraphLinear(
                hidden_dim=hidden_dim_for_update)
            self.update_for_global = UpdateLayerWithLinear(
                hidden_dim=hidden_dim_for_update)
    ...
    def __call__(self, atoms_feat, pair_feat, global_feat,
        ...
        concat_p_v = get_concated_edge_vec(
            a_f_d, p_f_d, g_f_d, bond_num, bond_idx, self.xp)
        update_p = self.update_for_atom(concat_p_v)

        # 3) Update the node vector
        concat_a_v = get_concated_node_vec(
            a_f_d, update_p, g_f_d, atom_num, bond_num, bond_idx, self.xp)
