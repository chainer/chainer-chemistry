...
def main(num_nodes, num_graphs, num_labels, out_prefix):
    ...
    * attach labels
    - 1. number of node "0"
    - 2. number of node "0" + subgraphs bonus
    ...
    for g in g_list:
        ...
        idx = np.where(u_elem == 0)
        if len(idx[0]) < 1:
            score = np.array([0])
        else:
            score = u_cnt[idx[0]]

        y_list_1.append( score )
    #
    ...
    for g in g_list:
        ...
        for idx, sg in enumerate(subgraphs):
            ...
            if g1g2.subgraph_is_isomorphic():
                score = score + subgraph_scores[idx]
        # end sg
        ...
        if len(idx[0]) < 1:
            ...
            score = score + u_cnt[idx[0]]

        y_list_2.append(score)
        #
    # end g
    ...
    def generate_TranValTest(g_list, y_list):
        ...
        for ii in range(0, num_train):
            ...
            As.append(nx.to_numpy_array(g) + np.eye(num_nodes))
            ys.append(y)
        dataset_train = NumpyTupleDataset(np.array(Xs, dtype=np.int32), np.array(As, dtype=np.float32), np.array(ys, dtype=np.float32))
        ...
        for ii in range(num_train, num_train+num_val):
            ...
            As.append(nx.to_numpy_array(g) + np.eye(num_nodes))
            ys.append(y)
        dataset_val = NumpyTupleDataset(np.array(Xs, dtype=np.int32), np.array(As, dtype=np.float32), np.array(ys, dtype=np.float32))
        ...
        dataset_test = NumpyTupleDataset(np.array(Xs, dtype=np.int32), np.array(As, dtype=np.float32), np.array(ys, dtype=np.float32))
    ...
    train_d, val_d, test_d = generate_TranValTest(g_list, y_list_1)
